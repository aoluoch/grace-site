...
const richTextOptions: Options = {
  ...
  renderNode: {
    [BLOCKS.PARAGRAPH]: (_, children) => (
      <p className="text-[0.98rem] leading-[1.55] text-[#222] sm:text-[1.03rem]">
        {children}
      </p>
    ),
    ...
    [BLOCKS.HEADING_4]: (_, children) => (
      <h6 className="text-base font-semibold text-[#1f2167]">{children}</h6>
    ),
    [BLOCKS.HEADING_5]: (_, children) => (
      <h6 className="text-sm font-semibold text-[#1f2167]">{children}</h6>
    ),
    [BLOCKS.HEADING_6]: (_, children) => (
      <h6 className="text-sm font-medium text-[#1f2167]">{children}</h6>
    ),
    ...
    <div className="relative pl-12">
      ...
      <article className="rounded-2xl bg-[#f3f3f3] px-5 py-4 shadow-[0_2px_10px_rgba(0,0,0,0.05)]">
        <div className="space-y-2">
          {documentToReactComponents(item.description, richTextOptions)}
        </div>
      </article>
    </div>
  );
...
function Declaration() {
    ...
    const fetchDeclarations = async () => {
      try {
        ...
        const mapped: DeclarationItem[] = response.items
          .map((item) => {
            const numberField = (item.fields as Record<string, unknown>)
              ?.number;
            const descriptionField = (item.fields as Record<string, unknown>)
              ?.description;
            ...
            if (
              ...
              !description ||
              description.nodeType !== BLOCKS.DOCUMENT
            ) {
              return null;
            }
  ...
  const [leftDeclarations, rightDeclarations] = useMemo(() => {
    ...
    return [
      declarations.slice(0, middleIndex),
      declarations.slice(middleIndex),
    ];
  }, [declarations]);
    ...
    <section className="bg-[#efefef] py-14 sm:py-16">
      <div className="mx-auto w-full max-w-285 px-4 sm:px-6 lg:px-8">
        <header className="mb-7">
          ...
          <p className="mt-1 text-[1rem] text-[#666] sm:text-[1.08rem]">
            Speak these over your life in Jesus’ Name.
          </p>
        </header>
        ...
        <div className="grid items-stretch gap-6 lg:grid-cols-[1fr_360px_1fr]">
          <div className="space-y-4">
            {isLoading && (
              <p className="text-[#4B4B67]">Loading declarations...</p>
            )}
