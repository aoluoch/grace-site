        ...
        <div className="grid gap-4 lg:grid-cols-[0.95fr_1.25fr] lg:gap-5">
          <article className="overflow-hidden border border-[#d0d0d0] bg-black shadow-[0_2px_10px_rgba(0,0,0,0.12)]">
            <iframe
              ...
              className="h-[240px] w-full sm:h-[320px] lg:h-full lg:min-h-[360px]"
          ...
          <article className="rounded-[8px] border border-[#d8d8d8] bg-[#f2f2f2] shadow-[0_2px_10px_rgba(0,0,0,0.08)]">
            <header className="bg-[#1f2167] px-4 py-2.5 sm:px-5">
              <h2 className="text-[1rem] font-semibold tracking-[0.02em] text-white sm:text-[1.07rem]">DOM PAYMENT DETAILS</h2>
            </header>

            <div className="grid gap-4 px-4 py-4 sm:px-5 sm:py-5 lg:grid-cols-[1.02fr_0.98fr]">
              <div>
                <p className="text-[1.2rem] font-semibold leading-tight text-[#b8aa69] sm:text-[1.4rem]">THANK YOU FOR YOUR</p>
                <p className="inline-block bg-white px-1.5 py-0.5 text-[1.9rem] font-semibold leading-none tracking-[0.01em] text-[#1f2167] sm:text-[2.05rem]">
                  OFFERING
                </p>
                      ...
                      <div className="mt-2 space-y-1.5 overflow-x-auto">
                        {block.entries.map((entry) => (
                          <p
                            ...
                            className="min-w-fit whitespace-nowrap text-[0.84rem] font-semibold tracking-[0.01em] text-[#1f2167] sm:text-[0.92rem] xl:text-[0.98rem]"
                          >
              ...
              <aside className="rounded-[6px] border border-[#dbdbdb] bg-[#f5f5f5] px-3 py-3 sm:px-4">
                <div className="space-y-2 overflow-x-auto">
                  {sideEntries.map((entry, index) => (
                    <p
                      key={`${entry.label}-${index}`}
                      className="min-w-fit whitespace-nowrap text-[0.8rem] font-semibold tracking-[0.01em] text-[#1f2167] sm:text-[0.88rem] xl:text-[0.95rem]"
                    >
                      <span className="text-[#b8aa69]">{entry.label}:</span>{' '}
                      <span>{entry.value}</span>
                    </p>
