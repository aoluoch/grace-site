...
const EventComponent = () => {
  ...
  const eventCards = useMemo(() => {
    return events.map((event) => {
      const title = getFirstLocaleString(event.fields.title) ?? "Untitled Event"
  ...
  if (eventCards.length === 0) {
    return (
      <div className="py-12 text-center">
        <p className="text-[#4B4B67] text-lg">No events available at this time.</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      {eventCards.map((event) => (
        <div key={event.id} className="bg-white rounded-lg shadow-md overflow-hidden border border-[#ECECF3] hover:shadow-lg transition-shadow duration-300">
          <div className="flex flex-col md:flex-row">
            ...
            <div className="relative overflow-hidden md:w-2/5 lg:w-1/3 shrink-0 bg-[#ECECF3]">
              {event.imageUrl ? (
                ...
                <div className="w-full h-64 md:h-80 bg-linear-to-br from-[#ECECF3] to-[#D8D8E5] flex items-center justify-center">
                  <svg className="w-16 h-16 text-[#B8B8C8]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="p-6 md:p-8 flex-1 flex flex-col justify-center">
              <h3 className="text-2xl font-bold text-[#202163] mb-3 leading-tight">{event.title}</h3>
              {event.summary && (
                <p className="text-base text-[#4B4B67] mb-6 leading-relaxed line-clamp-3">{event.summary}</p>
              )}
              <Link
                to={`/events/${event.id}`}
